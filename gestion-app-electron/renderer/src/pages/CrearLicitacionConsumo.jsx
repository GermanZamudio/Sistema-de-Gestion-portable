...
export default function CrearLicitacionConsumo() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    const response = await window.api.post("/api/inventario/licitacion/consumo", licitacion);
    if (response.error) return alert("Error al guardar: " + response.error);
    alert("Licitación de consumo guardada correctamente.");
    setLicitacion({
      nombre: "",
      año: "",
      articulos: [
  ...
  return (
    <Container>
      <h1>Crear Licitación de Bienes de Consumo</h1>
